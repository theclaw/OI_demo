...
if(debug == False):
	if os.environ["SPLUNK_HOME"].find('\\') == -1:
		the_rad_File = open(os.environ["SPLUNK_HOME"] +  os.path.abspath('/etc/apps/telcodemo/bin/output/' + filename1), 'w')
		the_web_File = open(os.environ["SPLUNK_HOME"] +  os.path.abspath('/etc/apps/telcodemo/bin/output/' + filename2), 'w')
	else:
		the_rad_File = open(os.environ["SPLUNK_HOME"] +  ('\\etc\\apps\\telcodemo\\bin\\output\\' + filename1), 'w')
		the_web_File = open(os.environ["SPLUNK_HOME"] +  ('\\etc\\apps\\telcodemo\\bin\\output\\' + filename2), 'w')

# Define lib path
libPath = os.path.join(os.environ["SPLUNK_HOME"], 'etc','apps','telcodemo', 'bin', 'data')
...
while loop :
	...
	# Append to file
	if os.environ["SPLUNK_HOME"].find('\\') == -1:
		the_rad_File = open(os.environ["SPLUNK_HOME"] +  os.path.abspath('/etc/apps/telcodemo/bin/output/' + filename1), 'a')
		the_web_File = open(os.environ["SPLUNK_HOME"] +  os.path.abspath('/etc/apps/telcodemo/bin/output/' + filename2), 'a')
	else:
		the_rad_File = open(os.environ["SPLUNK_HOME"] +  ('\\etc\\apps\\telcodemo\\bin\\output\\' + filename1), 'a')
		the_web_File = open(os.environ["SPLUNK_HOME"] +  ('\\etc\\apps\\telcodemo\\bin\\output\\' + filename2), 'a')
	...
	if(debug):
		print(radstartline.rstrip())
		print(radstopline.rstrip())
		print(webline.rstrip())
	else:
